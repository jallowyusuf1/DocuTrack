...
import { ArrowLeft, Calendar, AlertCircle, Image as ImageIcon, Loader2, Camera } from 'lucide-react';
...
export default function AddDocument() {
  ...
  // Process image file (used by both file input and camera modal)
  const processImageFile = async (file: File) => {
    ...
    try {
      ...
      try {
        showToast('Aligning and enhancing image...', 'info');
        ...
        console.log(`Image enhanced from ${originalSize} to ${newSize}`);
        showToast(`Image aligned and enhanced to 8K quality`, 'success');
      } catch (enhanceError) {
        console.warn('Enhancement failed, using original:', enhanceError);
        // Fallback: compress if needed
        try {
          if (needsCompression(file, 2)) {
            const originalSize = formatFileSize(file.size);
            processedFile = await compressImage(file);
            const newSize = formatFileSize(processedFile.size);
            showToast(`Image optimized: ${originalSize} → ${newSize}`, 'success');
          } else {
            processedFile = file;
          }
        } catch (compressError) {
          console.warn('Compression also failed, using original:', compressError);
          processedFile = file;
        }
      }
  ...
  return (
    <div className="min-h-screen pb-[140px] relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        {/* Form */}
        <form onSubmit={handleFormSubmit} className="px-4 py-6 space-y-5 md:space-y-6">
          {/* Image Preview */}
          <div className="space-y-3">
            {imagePreview ? (
              ...
              >
                {isCompressing ? (
                  <>
                    <Loader2 className="w-12 h-12 mb-3 animate-spin" style={{ color: '#A78BFA' }} />
                  ...
                  <>
                    <Camera className="w-12 h-12 mb-3" style={{ color: '#A78BFA' }} />
                    <button
                      type="button"
                      onClick={() => {
                        triggerHaptic('medium');
                      ...
                      className="glass-btn-primary px-6 py-3 rounded-xl font-semibold flex items-center gap-2"
                    >
                      <Camera className="w-5 h-5" />
                      Take Photo or Upload
                    </button>
                    <p className="text-xs text-glass-secondary mt-3">
          ...
          {/* Issue Date */}
          <div className="mt-1">
            <label className="block text-sm md:text-[15px] font-semibold text-white mb-3 mt-1">
              Issue Date
            </label>
            <motion.button
              whileTap={{ scale: 0.98 }}
              ...
              className={`
                w-full h-[52px] md:h-[56px] px-4 rounded-xl
        ...
        >
          <Button
            ...
            className="h-[52px] md:h-[56px] text-base md:text-[19px] font-semibold"
          >
            {isSaving ? (
              <>
