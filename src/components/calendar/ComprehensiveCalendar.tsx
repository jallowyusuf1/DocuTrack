...
export default function ComprehensiveCalendar({
  ...
  // Handle adding important date (for main Add button)
  const handleAddImportantDate = (date?: Date) => {
    const dateParam = date || selectedDate || new Date();
    navigate(`/dates?add=true&date=${format(dateParam, 'yyyy-MM-dd')}`);
  };

  // Handle adding document (for date panel buttons)
  const handleAddDocument = (date?: Date) => {
    if (onAddDocument) {
      // Use callback if provided (Calendar page will handle navigation)
      onAddDocument(date || selectedDate || new Date());
    } else {
      // Default: Navigate to add document page
      const dateParam = date || selectedDate || new Date();
      ...
      <AnimatePresence>
        {showDatePanel && selectedDate && (
          <DatePanel
            ...
            onAddEvent={() => handleAddDocument(selectedDate)}
            isMobile={!isDesktop}
          />
        )}
      </AnimatePresence>
