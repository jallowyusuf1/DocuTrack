  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        {/* Modal */}
        <motion.div
          className="relative w-full max-w-2xl rounded-3xl overflow-hidden max-h-[90vh] flex flex-col"
          style={{
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {Object.entries(groupedShortcuts).map(([category, categoryShortcuts]) => (
              <section key={category}>
                <h3 className="text-white font-semibold text-sm mb-3 uppercase tracking-wider text-white/60">
                  {category}
                </h3>
                      ...
                      <div className="flex items-center gap-1.5">
                        {shortcut.keys.map((key, keyIndex) => (
                          <span key={keyIndex} className="flex items-center">
                            <kbd className="px-2.5 py-1 rounded-lg bg-white/10 border border-white/20 text-white/90 text-xs font-mono flex items-center gap-1">
