...
import { staggerContainer, staggerItem, fadeInUp, triggerHaptic } from '../../utils/animations';
...
export default function Documents() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('searchDocuments', handleSearch as EventListener);
    return () => window.removeEventListener('searchDocuments', handleSearch as EventListener);
  }, []);
  ...
  const handleMarkRenewed = async (document: Document) => {
    // This will be handled by the DocumentCard component's renewal modal
  ...
  return (
    <div className="min-h-screen pb-[72px] relative overflow-hidden">
      ...
      {/* Category Tabs */}
      <div className="py-3 lg:max-w-[1280px] lg:mx-auto lg:px-8">
        <CategoryTabs
        ...
        {/* Sort/Filter/View Toggle Bar */}
        <div className="flex items-center justify-between px-4 pb-3 gap-3 lg:max-w-[1280px] lg:mx-auto lg:px-8">
        ...
        <div
          ...
          className="px-4 pb-4 lg:max-w-[1280px] lg:mx-auto lg:px-8"
        >
        {loading ? (
          <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4' : 'grid grid-cols-1 gap-4'}>
            {Array.from({ length: 6 }).map((_, i) => (
              <Skeleton key={i} className={`w-full rounded-3xl ${viewMode === 'grid' ? 'min-h-[500px] md:min-h-[320px] lg:min-h-[293px]' : 'min-h-[120px]'}`} />
            ))}
          </div>
        ) : error ? (
          ...
          >
            {filteredAndSortedDocuments.map((document) => (
              <motion.div key={document.id} variants={staggerItem} className="w-full">
